...
# edna.dat from AIMS link
edna.dat <- read.csv("data/eDNA data_ALL.csv") |>
  mutate(Conc_mean = as.numeric(Conc_mean)) #|>
  #filter(Collection.organisation == "AIMS")

# # edna.dat <- read_excel("data/CCIP-RT-02 eDNA data.xlsx", sheet =1)
# edna.dat.others <- read.csv("data/eDNA data_ALL.csv") |>
#   mutate(Conc_mean = as.numeric(Conc_mean)) |>
#   filter(!Collection.organisation == "AIMS")
...
```{r}
ggplot(reef_any, aes(x = perc_pos_reef, y = cpue_reef, fill = Collection.org,, color=Collection.org)) +
  geom_point(size = 2) +
...
ggplot(reef_any, aes(x = log(conc_mean_reef), y = cpue_reef,fill = Collection.org,, color=Collection.org)) +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(
...
ggplot(reef_any, aes(x = log1p(conc_mean_reef), y = cpue_reef,fill = Collection.org, color=Collection.org)) +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(x = "log1p(mean concentration)", y = "CPUE") +
  theme_bw()

ggplot(reef_any, aes(x = asinh(conc_mean_reef), y = cpue_reef,fill = Collection.org, color=Collection.org)) +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = TRUE) +
...
```{r}
ggplot(reef_before, aes(x = perc_pos_reef, y = cpue_reef,fill = Collection.org, color=Collection.org)) +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(
    x = "% eDNA positive (reef-level)",
...
ggplot(reef_before_closest, aes(x = perc_pos_reef, y = cpue_reef,fill = Collection.org, color=Collection.org)) +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(
    x = "% eDNA positive (reef-level)",
...
# ----- Plot -----
ggplot(dat, aes(x = perc_pos_reef, y = obs_cpue,fill = Collection.org, color=Collection.org)) +
...
res <- make_confusion(dat_evt %>% filter(Collection.org=="AIMS"), perc_thresh, cpue_thresh)
print(res$metrics)

p1 = plot_confusion(res,
  title = sprintf("AIMS (perc ≥ %.1f, CPUE ≥ %.3f)", perc_thresh, cpue_thresh)
)

res.others <- make_confusion(dat_evt %>% filter(Collection.org=="Other"), perc_thresh, cpue_thresh)
print(res.others$metrics)

p2 = plot_confusion(res.others,
  title = sprintf("Other (perc ≥ %.1f, CPUE ≥ %.3f)", perc_thresh, cpue_thresh)
)
p1/p2
```
...
# pick the best %pos threshold on TRAINING data (max F1, tie-break: simplest)
best_threshold_train <- function(x_train, perc_grid, cpue_thr) {
  res <- tibble(perc_thresh = perc_grid) %>%
    mutate(out = map(perc_thresh, ~ metrics_for(.x, cpue_thr, x_train)),
           F1  = vapply(out, `[[`, numeric(1), "F1")) %>%
    dplyr::select(-out)

  # choose smallest threshold among ties (more conservative; change if you prefer)
  best <- res %>% filter(F1 == max(F1, na.rm = TRUE)) %>% slice_min(perc_thresh, with_ties = FALSE)
  best$perc_thresh
}
...
cpue_thr  <- 0.02                  # <-- set your management CPUE threshold
perc_grid <- seq(0, 100, by = 1)
cpue_levels <- c(0, 0.02, 0.04, 0.06, 0.08, 0.10)
# simple stratified fold assignment
dat_evt <- dat_evt %>%
  mutate(actual = cpue >= cpue_thr)


# repeated CV: tune these
k <- 5
R <- 200   # repeats (e.g., 100–500)

folds <- vfold_cv(dat_evt, v = k, repeats = R, strata = actual)
...
cv_long <- folds %>%
  ...
  mutate(
    out = pmap(list(perc_thresh, assess),
               \(p, d) metrics_for(p, cpue_thr, d)),
    F1        = vapply(out, `[[`, numeric(1), "F1"),
    Accuracy  = vapply(out, `[[`, numeric(1), "Accuracy"),
...
target <- best$F1_mean - best$F1_se
best_1se <- cv_sum %>%
  filter(F1_mean >= target) %>%
  slice_max(perc_thresh, n = 1, with_ties = FALSE)

best_1se

ggplot(cv_sum, aes(x = perc_thresh, y = F1_mean)) +
  geom_line() +
  geom_ribbon(aes(ymin = F1_mean - F1_se, ymax = F1_mean + F1_se), alpha = 0.2) +
  geom_vline(xintercept = best$perc_thresh, linetype = 2) +
  geom_vline(xintercept = best_1se$perc_thresh, linetype = 3) +
  labs(x = "eDNA % positive threshold", y = "CV mean F1 (±1 SE)",
       title = sprintf("Repeated %d-fold CV (R=%d) tuning of %%pos threshold; CPUE≥%.3f", k, R, cpue_thr)) +
  theme_bw()


p_star <- best_1se$perc_thresh  # or best$perc_thresh


oof <- folds %>%
  mutate(
    assess = map(splits, assessment),
    oof_df = map(assess, ~ tibble(
      pred = .x$perc_pos >= p_star,
      act  = .x$cpue     >= cpue_thr
    ))
  ) %>%
  dplyr::select(id, id2, oof_df) %>%
  unnest(oof_df)

# Confusion counts across all held-out predictions
TP <- sum(oof$pred &  oof$act)
FP <- sum(oof$pred & !oof$act)
TN <- sum(!oof$pred & !oof$act)
FN <- sum(!oof$pred &  oof$act)
...
c(Accuracy=acc, Precision=prec, Recall=rec, F1=f1, perc_thresh=p_star)



```

```{r Plot CV confusion matrix}
# thresholds
perc_star <- p_star   # from your tuning
cpue_thr  <- cpue_thr # fixed management threshold

cm_full <- make_confusion(dat_evt, perc_star, cpue_thr)

cm_full$metrics
plot_confusion(cm_full,
  title = sprintf("FULL data: perc_pos ≥ %.0f, CPUE ≥ %.3f", perc_star, cpue_thr)
)



```
```{r Gneerate for multiple CPUEs}

cpue_levels <- c(0.01, 0.02, 0.04, 0.06, 0.08, 0.10)

run_cv_for_cpue <- function(dat_evt, cpue_thr, perc_grid, k = 5, R = 200) {

  dat2 <- dat_evt %>%
    mutate(actual = if (cpue_thr == 0) (cpue > 0) else (cpue >= cpue_thr))

  # if only one class, stratification is impossible
  strat_ok <- length(unique(dat2$actual)) > 1

  folds <- if (strat_ok) {
    vfold_cv(dat2, v = k, repeats = R, strata = actual)
  } else {
    vfold_cv(dat2, v = k, repeats = R)
  }

  # evaluate every threshold on every assessment set
  cv_long <- folds %>%
  ...
  # tune threshold: maximise mean out-of-fold F1
  cv_sum <- cv_long %>%
    group_by(perc_thresh) %>%
    summarise(
      F1_mean = mean(F1),
  ...
  # CV performance at tuned threshold (per split, then summarise)
  perf_split <- folds %>%
    mutate(
      assess = map(splits, assessment),
      m = map(assess, ~ {
        out <- metrics_for(p_star, cpue_thr, .x)
        tibble(F1 = out["F1"], Accuracy = out["Accuracy"],
  ...
  # confusion matrix on FULL dataset using tuned threshold
...
# Summarise performance across CPUE levels
summary_table <- map_dfr(results, "perf_sum") %>%
  mutate(
    F1_mean_sd   = sprintf("%.3f ± %.3f", F1_mean, F1_sd),
    Acc_mean_sd  = sprintf("%.3f ± %.3f", Acc_mean, Acc_sd),
    Prec_mean_sd = sprintf("%.3f ± %.3f", Prec_mean, Prec_sd),
    Rec_mean_sd  = sprintf("%.3f ± %.3f", Rec_mean, Rec_sd),
  ...
  dplyr::select(cpue_thr, perc_star,
         F1_mean_sd, Acc_mean_sd, Prec_mean_sd, Rec_mean_sd,
         F1_mean_se, Acc_mean_se, Prec_mean_se, Rec_mean_se)
...
cm_all <- map_dfr(results, function(r) {
  r$cm_full$cm %>%
    mutate(
      cpue_thr = r$cpue_thr,
      perc_star = r$perc_star
    )
}) %>%
  mutate(panel = paste0("CPUE ≥ ", cpue_thr, "\n%pos* = ", perc_star))

ggplot(cm_all, aes(x = pred, y = actual, fill = n)) +
  geom_tile(color = "grey30") +
  geom_text(aes(label = label), size = 4) +
  facet_wrap(~ panel, ncol = 3,
             labeller = labeller(cpue_thr = function(x) paste0("CPUE ≥ ", x))) +
  scale_fill_gradient(low = "white", high = "steelblue") +
  labs(x = "Prediction from eDNA (% pos threshold tuned by CV)",
       y = "Ground truth from CPUE",
       fill = "Count",
       title = "Confusion matrices using CV-tuned %pos thresholds") +
  coord_fixed() +
  theme_bw() +
  theme(panel.grid = element_blank())
...
ggplot(cm_all, aes(x = pred, y = actual, fill = row_prop)) +
  geom_tile(color = "grey85", linewidth = 0.4) +
  geom_text(aes(label = label), size = 4, color = "black") +
  facet_wrap(~ panel, ncol = 3) +
  scale_fill_viridis_c(option = "mako", direction = -1,begin=0.25, name = "Count") +
  # scale_fill_viridis_c(option = "cividis", direction = -1,begin=0.2, name = "Count") +
  labs(
    x = "Prediction from eDNA (% pos)",
    y = "Ground truth from CPUE",
    title = "Confusion matrices with CV-tuned thresholds"
  ) +
  coord_fixed() +
  theme_bw() +
  theme(
...
dat <- reef_before %>%
  transmute(
    ...
    conc     = conc_mean_reef,
    conc_t   = log1p(conc_mean_reef),
    Collection.org = Collection.org# <-- transformation
  ) %>%
  filter(!is.na(cpue), !is.na(counts), !is.na(effort), effort > 0,
         !is.na(perc_pos), !is.na(conc_t)) #%>%
  #filter(Collection.org == "AIMS")
library(glmmTMB)

m_nb <- glmmTMB(
  counts ~ perc_pos + conc_t + offset(log(effort)),
  family = nbinom2(),
  data = dat
)
summary(m_nb)
...
# model (interaction so conc helps separate high CPUE regimes)
fit_nb <- function(df) {
  glmmTMB(
    counts ~ perc_pos * conc_t + offset(log(effort)),
    family = nbinom2(),
...
metrics_from_score <- function(score, actual) {
  TP <- sum(score &  actual)
  FP <- sum(score & !actual)
  TN <- sum(!score & !actual)
  FN <- sum(!score &  actual)
...
tune_for_cpue <- function(dat, cpue_thr, k = 5, R = 200, n_grid = 200, seed = 1) {
  ...
  folds <- if (length(unique(dat2$actual)) > 1) {
    vfold_cv(dat2, v = k, repeats = R, strata = actual)
  } else {
    vfold_cv(dat2, v = k, repeats = R)
  }

  # OOF predictions per split
  oof <- folds %>%
    mutate(
      te = map(splits, assessment),
      tr = map(splits, analysis),
      fit = map(tr, fit_nb),
      mu = map2(fit, te, ~ predict(.x, newdata = .y, type = "response")), # mean counts
      score_cpue = map2(mu, te, ~ .x / .y$effort),                        # predicted CPUE score
      oof_df = map2(te, score_cpue, ~ tibble(
        cpue = .x$cpue,
        actual = .x$cpue >= cpue_thr,
        score_cpue = .y
      ))
    ) %>%
    select(id, id2, oof_df) %>%
    unnest(oof_df)

  # Threshold grid on the score (predicted CPUE)
  thr_grid <- quantile(oof$score_cpue, probs = seq(0, 1, length.out = n_grid), na.rm = TRUE)

  # Evaluate each score threshold on all OOF rows (note: repeats replicate rows; OK for tuning)
  grid <- tibble(score_thr = thr_grid) %>%
    mutate(
      out = map(score_thr, ~ {
        pred <- oof$score_cpue >= .x
        as.list(metrics_from_score(pred, oof$actual))
      }),
      F1 = map_dbl(out, "F1"),
      Accuracy = map_dbl(out, "Accuracy"),
      Precision = map_dbl(out, "Precision"),
      Recall = map_dbl(out, "Recall")
    ) %>%
    select(-out)

  best <- grid %>% slice_max(F1, n = 1, with_ties = FALSE)
  s_star <- best$score_thr

  # Split-wise metrics (best practice for mean±SD reporting)
  split_metrics <- folds %>%
    mutate(
      te = map(splits, assessment),
      fit = map(splits, ~ fit_nb(analysis(.x))),
      score = map2(fit, te, ~ predict(.x, newdata = .y, type = "response") / .y$effort),
      actual = map(te, ~ .x$cpue >= cpue_thr),
      m = map2(score, actual, ~ {
        pred <- .x >= s_star
        out <- metrics_from_score(pred, .y)
  ...
  list(perf = perf, score_thr = s_star)
}

boundary_curve <- function(fit_full, score_thr_star, perc_seq, effort_ref) {
  b <- fixef(fit_full)$cond
  b0 <- unname(b["(Intercept)"])
  b1 <- unname(b["perc_pos"])
  b2 <- unname(b["conc_t"])
  b3 <- unname(b["perc_pos:conc_t"])

  rhs <- log(score_thr_star * effort_ref) - log(effort_ref)  # offset cancels, but keep explicit
  # Actually: eta = b0 + b1*p + b2*c + b3*p*c + log(effort_ref)
  # score_thr_star = exp(eta)/effort_ref => log(score_thr_star) = b0 + b1*p + b2*c + b3*p*c
  # so rhs = log(score_thr_star) - b0 - b1*p
  # Solve: (b2 + b3*p) * c = log(score_thr_star) - b0 - b1*p

  tibble(perc_pos = perc_seq) %>%
    mutate(
      denom = (b2 + b3 * perc_pos),
      conc_t = (log(score_thr_star) - b0 - b1 * perc_pos) / denom,
      conc_mean = pmax(expm1(conc_t), 0)
    ) %>%
    filter(is.finite(conc_mean), denom != 0)
}
...
# Tune score thresholds by repeated CV for each CPUE definition
res <- map(cpue_levels, ~ tune_for_cpue(dat, .x, k = k, R = R, seed = 1))

summary_table <- map_dfr(res, "perf") %>%
  mutate(
    F1_mean_sd   = sprintf("%.3f ± %.3f", F1_mean, F1_sd),
    Acc_mean_sd  = sprintf("%.3f ± %.3f", Acc_mean, Acc_sd),
    Prec_mean_sd = sprintf("%.3f ± %.3f", Prec_mean, Prec_sd),
  ...
  select(cpue_thr, score_thr_star,
         F1_mean_sd, Acc_mean_sd, Prec_mean_sd, Rec_mean_sd,
         F1_mean_se, Acc_mean_se, Prec_mean_se, Rec_mean_se)
...
fit_full <- fit_nb(dat)
effort_ref <- median(dat$effort, na.rm = TRUE)
perc_seq <- seq(min(dat$perc_pos), max(dat$perc_pos), length.out = 200)

boundaries <- map2_dfr(cpue_levels, map_dbl(res, "score_thr"), ~ {
  boundary_curve(fit_full, .y, perc_seq, effort_ref) %>%
    mutate(cpue_thr = .x, score_thr = .y)
})


library(ggplot2)

ggplot(boundaries, aes(x = perc_pos, y = conc_mean, colour = factor(cpue_thr))) +
  geom_line(linewidth = 1) +
  scale_y_continuous(trans = scales::pseudo_log_trans(sigma = 1)) +
  labs(
    x = "% eDNA positive (reef-level)",
    y = "Mean concentration (threshold boundary)",
    colour = "CPUE threshold",
    title = "Model-derived decision boundaries (best combinations of %pos & conc)"
  ) +
  theme_bw()
...
## 2D Grid Search for AND  / OR combination rules CV
```{r}
library(dplyr)
library(tidyr)
library(purrr)
...
# tune on transformed concentration; then convert back for reporting/ops
conc_grid_t <- seq(min(dat_evt$conc_t, na.rm=TRUE),
                   quantile(dat_evt$conc_t, 0.99, na.rm=TRUE),
                   length.out = 60)
...
rule <- "and"   # "and" is the rectangular rule; switch to "or" if you want

# returns best (perc_star, conc_t_star) on TRAINING data (max F1); conservative tie-break = higher thresholds
best_pair_train <- function(df, cpue_thr, perc_grid, conc_grid_t, rule = c("and","or")) {
  rule <- match.arg(rule)
  ...
  perc_ge <- outer(df$perc_pos, perc_grid, `>=`)   # n x P
  conc_ge <- outer(df$conc_t,   conc_grid_t, `>=`) # n x C
  ...
  # tie-break: pick higher thresholds (more conservative)
  idx <- idx[order(idx[,1], idx[,2], decreasing = TRUE), , drop = FALSE][1, ]

  list(
    perc_star   = perc_grid[idx[1]],
    conc_t_star = conc_grid_t[idx[2]],
    F1_train    = max_f1
  )
}

metrics_pair <- function(df, cpue_thr, perc_star, conc_t_star, rule = c("and","or")) {
  rule <- match.arg(rule)
  actual <- df$cpue >= cpue_thr
  pred <- if (rule == "and") {
    (df$perc_pos >= perc_star) & (df$conc_t >= conc_t_star)
  } else {
    (df$perc_pos >= perc_star) | (df$conc_t >= conc_t_star)
  }
  ...
  tibble(F1=f1, Accuracy=acc, Precision=prec, Recall=rec)
}


run_cv_2d <- function(dat, cpue_thr, perc_grid, conc_grid_t, k=5, R=200, rule="and", seed=1) {
  set.seed(seed)
  ...
  split_res <- map_dfr(folds$splits, function(spl) {
    ...
    best <- best_pair_train(tr, cpue_thr, perc_grid, conc_grid_t, rule = rule)
    met  <- metrics_pair(te, cpue_thr, best$perc_star, best$conc_t_star, rule = rule)

    tibble(
      cpue_thr = cpue_thr,
      perc_star = best$perc_star,
      conc_t_star = best$conc_t_star,
      conc_mean_star = pmax(expm1(best$conc_t_star), 0)
    ) %>% bind_cols(met)
  })

  # choose operational thresholds as medians across resamples
  thr <- split_res %>%
    summarise(
      perc_star = median(perc_star),
      conc_t_star = median(conc_t_star),
      conc_mean_star = median(conc_mean_star),
      .groups = "drop"
    )

  # performance summary (mean ± SD and SE across resamples)
  perf <- split_res %>%
    summarise(
      cpue_thr = cpue_thr,
      perc_star = thr$perc_star,
      conc_mean_star = thr$conc_mean_star,
  ...
  list(split_res = split_res, thr = thr, perf = perf)
}

```

```{r Run CV 2D}
results <- map(cpue_levels, ~ run_cv_2d(dat_evt, .x, perc_grid, conc_grid_t, k=k, R=R, rule=rule, seed=1))

summary_table <- map_dfr(results, "perf") %>%
  mutate(
    F1_mean_sd   = sprintf("%.3f ± %.3f", F1_mean, F1_sd),
    Acc_mean_sd  = sprintf("%.3f ± %.3f", Acc_mean, Acc_sd),
    Prec_mean_sd = sprintf("%.3f ± %.3f", Prec_mean, Prec_sd),
  ...
  select(cpue_thr, perc_star, conc_mean_star,
         F1_mean_sd, Acc_mean_sd, Prec_mean_sd, Rec_mean_sd,
         F1_mean_se, Acc_mean_se, Prec_mean_se, Rec_mean_se)

summary_table

make_confusion_2d <- function(dat, perc_star, conc_t_star, cpue_thr, rule = c("and","or")) {
  rule <- match.arg(rule)

  classified <- dat %>%
    mutate(
      pred = case_when(
        rule == "and" ~ if_else(perc_pos >= perc_star & conc_t >= conc_t_star, "Pred +", "Pred -"),
        TRUE          ~ if_else(perc_pos >= perc_star | conc_t >= conc_t_star, "Pred +", "Pred -")
      ),
  ...
  list(
    cm = cm,
    metrics = tibble(cpue_thr=cpue_thr, perc_star=perc_star, conc_mean_star=pmax(expm1(conc_t_star),0),
                     Accuracy=acc, Precision=prec, Recall=rec, F1=f1)
  )
}

# pull tuned thresholds
thr_df <- map_dfr(results, "thr") %>% mutate(cpue_thr = cpue_levels)

cm_all <- pmap_dfr(list(thr_df$cpue_thr, thr_df$perc_star, thr_df$conc_t_star), function(cp, p, c) {
  obj <- make_confusion_2d(dat_evt, perc_star = p, conc_t_star = c, cpue_thr = cp, rule = rule)
  obj$cm %>%
    mutate(panel = paste0("CPUE ≥ ", cp, "\n%pos*=", round(p,1), "  conc*≈", round(pmax(expm1(c),0),1)))
})

ggplot(cm_all, aes(pred, actual, fill = row_prop)) +
  geom_tile(color = "grey85", linewidth = 0.4) +
  geom_text(aes(label = label), size = 4, color = "black") +
  facet_wrap(~ panel, ncol = 3) +
  scale_fill_viridis_c(option = "mako", direction = -1, begin =0.25,limits = c(0,1),
                       labels = scales::percent, name = "Row %") +
  labs(
    x = "Prediction (2D rule)",
    y = "Ground truth (CPUE)",
    title = sprintf("2D rectangular rule (%s): Pred+ if %%pos and conc exceed thresholds", toupper(rule))
  ) +
  coord_fixed() +
  theme_bw() +
  theme(panel.grid = element_blank())
