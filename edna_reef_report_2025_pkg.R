...
thresh_2d <- map_dfr(cpue_boundaries, function(cthr) {
    bp <- best_pair(dat_glmm, cthr, perc_grid_thr, conc_grid_t)
    tibble(
        cpue_thr       = cthr,
...
get_thr_local <- function(thr) {
    res <- thresh_2d |> filter(cpue_thr == thr)
    if (nrow(res) == 0) stop("Threshold not found")
    list(perc_star = res$perc_star[1], conc_t_star = res$conc_t_star[1])
}

thr_01 <- get_thr_local(0.01) # boundary between <0.01 and 0.01-0.04
thr_04 <- get_thr_local(0.04) # boundary between 0.01-0.04 and 0.04-0.08
thr_08 <- get_thr_local(0.08) # boundary between 0.04-0.08 and >0.08
...
make_popup_local <- function(reef, date_edna, perc_pos, conc_mean, n_samples, cpue_category, region, tumra,
...
target_popups <- unname(with(
    edna_target,
    mapply(make_popup_local, ReefName, date_edna, perc_pos, conc_mean,
...
non_target_popups <- unname(with(
    edna_not_target_map,
    mapply(make_popup_local, Reef, date_edna, perc_pos, conc_mean,
